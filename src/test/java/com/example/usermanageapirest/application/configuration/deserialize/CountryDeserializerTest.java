...
class CountryDeserializerTest {
	...
	public void when_cannot_get_int_value_then_exception_is_thrown() throws IOException {
		...
		assertThrows(ValueDeserializationException.class,
			()->this.deserializer.deserialize(this.jsonParser,context));
	}
	...
	public void when_repository_is_used_then_data_is_returned() throws IOException {
		...
		Mockito.doReturn(Optional.of(new CountryBuilder().build(1))).when(this.repository).find(ArgumentMatchers.anyInt());
